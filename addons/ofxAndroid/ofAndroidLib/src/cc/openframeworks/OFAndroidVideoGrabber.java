...
Camera.PreviewCallback {
	...
	public void setTexture(int texID) {
		if (supportsTextureRendering()) {
			try {
				...
				Class<?> surfaceTextureClass = Class
						.forName("android.graphics.SurfaceTexture");
				Constructor<?> constructor = surfaceTextureClass
						.getConstructor(int.class);
				surfaceTexture = constructor.newInstance(texID);

				// set texture as preview surface for camera
				Method setPreviewTexture = camera.getClass().getMethod(
						"setPreviewTexture", surfaceTextureClass);
	...
	public void initGrabber(int w, int h, int _targetFps, int texID) {
		...
		Log.i("OF", "Grabber default preview size: "
				+ config.getPreviewSize().width + ","
		...
		try {
			Method setRecordingHint = config.getClass().getMethod(
					"setRecordingHint", boolean.class);
		...
		if (width != w || height != h)
			Log.w("OF",
					"camera size different than asked for, resizing (this can slow the app)");
		...
		bufferSize = bufferSize
				* ImageFormat.getBitsPerPixel(config.getPreviewFormat()) / 8;
	...
	public void update() {
		if (supportsTextureRendering()) {
			...
			try {
				surfaceTextureClass = Class
						.forName("android.graphics.SurfaceTexture");
				Method updateTexImage = surfaceTextureClass
						.getMethod("updateTexImage");
				if (surfaceTexture != null)
					updateTexImage.invoke(surfaceTexture);
			} catch (Exception e) {
	...
	public void getTextureMatrix(float[] mtx) {
		...
		try {
			surfaceTextureClass = Class
					.forName("android.graphics.SurfaceTexture");
			Method getTransformMatrix = surfaceTextureClass.getMethod(
					"getTransformMatrix", float[].class);
			if (surfaceTexture != null)
				getTransformMatrix.invoke(surfaceTexture, mtx);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void onPreviewFrame(byte[] data, Camera camera) {
		...
		// Log.i("OF", "size: " + camera.getParameters().getPreviewSize().width
	...
	public void run() {
		...
		try {
			addBufferMethod = Camera.class.getMethod("addCallbackBuffer",
					byte[].class);
			addBufferMethod.invoke(camera, buffer);
			Camera.class.getMethod("setPreviewCallbackWithBuffer",
					Camera.PreviewCallback.class).invoke(camera, this);
			Log.i("OF", "setting camera callback with buffer");
		} catch (SecurityException e) {
			Log.e("OF",
					"security exception, check permissions in your AndroidManifest to acces to the camera",
					e);
		} catch (NoSuchMethodException e) {
			try {
				Camera.class.getMethod("setPreviewCallback",
						Camera.PreviewCallback.class).invoke(camera, this);
				...
				Log.e("OF",
						"cannot create callback, the camera can only be used from api v7",
						e1);
			}
		} catch (Exception e) {
